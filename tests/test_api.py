...
async def _fake_reply(self, **kwargs):
    return GeminiTextResult(text="4", model=kwargs.get("model") or self.text_model, prompt_tokens=3, completion_tokens=1)


async def _fake_chat(self, **kwargs):
    return GeminiTextResult(text="support", model=kwargs.get("model") or self.text_model, prompt_tokens=4, completion_tokens=1)


async def _fake_image(self, **kwargs):
    return GeminiTextResult(text="a red square", model=kwargs.get("model") or self.image_model, prompt_tokens=2, completion_tokens=1)


async def _fake_tts(self, request):
    path = Path(self.audio_dir) / "tts-test.wav"
    path.write_bytes(b"RIFF0000WAVEfmt ")
    return AudioTtsArtifact(path=path, filename=path.name, model="gemini-2.5-flash", voice=request.voice, duration_seconds=1.0)


async def _fake_stt(self, **kwargs):
    from nexus.audio.models import AudioSttResponse

    return AudioSttResponse(text="hello world", raw_text="hello world", enhanced=kwargs.get("enhance", False), model=self.client.audio_model, language="en")


async def _fake_speech(self, **kwargs):
    return GeminiAudioResult(audio_bytes=b"RIFF0000WAVEfmt ", mime_type="audio/wav", model=kwargs.get("model") or self.audio_model, prompt_tokens=1, completion_tokens=1)
...
def test_api_routes(monkeypatch):
    ...
    monkeypatch.setattr("nexus.audio.service.AudioService.health", lambda self: {"ok": True, "providers": {"gemini": True}, "models": {"tts": "gemini-2.5-flash", "stt": "gemini-2.5-flash"}})
