...
import { parseEml } from '../enrichment/submissions/eml.js';
import { PROMPT_VERSION_V1 } from '../enrichment/submissions/extraction.js';
import type { Candidate } from '../enrichment/submissions/extraction.js';
import { ExtractionError, callOpenRouter } from '../enrichment/submissions/llm.js';
...
router.openapi(ingestRoute, async (c) => {
  ...
  if (contentType.startsWith('multipart/form-data')) {
    const {
      ...
      model: uploadModel,
      prompt_version,
    } = c.req.valid('form') as z.infer<typeof IngestUploadSchema>;
    ...
    const {
      ...
      model: jsonModel,
      prompt_version,
    } = c.req.valid('json') as z.infer<typeof IngestInputSchema>;
  ...
  try {
    for (const doc of texts) {
      ...
      const rawId = ingestService.insertExtractionRawRecord(
        runId,
        'api_ingest',
        'api_ingest',
        String(hashText(doc)),
      ...
      for (const candidate of result.packet.candidates) {
        ingestService.saveExtractionResult(
          runId,
          'api_ingest',
          candidate,
          result.modelName,
