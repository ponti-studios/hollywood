...
function resolveProvider(provider: LlmProvider): ProviderRequest {
  if (provider === "ollama") {
    return { url: `${env.OLLAMA_BASE_URL}/chat/completions`, headers: { "Content-Type": "application/json" } };
  }
...
function buildJsonSchema(): Record<string, unknown> {
  const relatedToSchema = {
    ...
    properties: {
      name: { type: "string", description: "The name of the title, company, or person this person is connected to." },
      ...
      character: {
        anyOf: [
          { type: "string", description: "The character name, only when relationship is 'actor' and type is 'title'." },
          { type: "null" },
        ],
      },
    },
