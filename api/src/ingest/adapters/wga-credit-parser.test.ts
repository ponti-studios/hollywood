...
import { parseSummaryFormatCredits, parseTableFormatCredits, parseWgaProfileCredits } from "./wga-credit-parser.js";
...
describe("parseSummaryFormatCredits", () => {
  it("parses the 'N Credits' summary section", () => {
    ...
    expect(results).toContainEqual({ title: "Baby Daddy", category: "Series", role: "writer", count: 13 });
    expect(results).toContainEqual({ title: "Wizards Of Waverly Place", category: "Series", role: "writer", count: 18 });
    expect(results).toContainEqual({ title: "Night Court", category: "Series", role: "writer", count: 2 });
  });
...
describe("parseTableFormatCredits", () => {
  ...
  it("captures multiple distinct roles on the same title (e.g. teleplay + story)", () => {
    ...
    expect(afterShock).toContainEqual({ title: "After Shock", category: "Pilot/Pending Series", role: "teleplay" });
    expect(afterShock).toContainEqual({ title: "After Shock", category: "Pilot/Pending Series", role: "story" });

    const zappedAgain = results.filter((r) => r.title === "Zapped Again");
    expect(zappedAgain).toContainEqual({ title: "Zapped Again", category: "Television", role: "teleplay" });
    expect(zappedAgain).toContainEqual({ title: "Zapped Again", category: "Television", role: "story" });
  });
  ...
  it("does not fire on a profile with no table section", () => {
    ...
    const text = fixture("ben_montanio.txt");
    const results = parseTableFormatCredits(text);
    expect(results.every((r) => ["The Wizards Return: Alex vs. Alex", "Wendy Wu: Homecoming Warrior", "After Shock", "Zapped Again"].includes(r.title))).toBe(true);
  });
});
