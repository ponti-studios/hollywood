...
describe("SubmissionService", () => {
  ...
  function createBaseData() {
    ...
    rawRecordRepo.insertOne({
      id: docId,
      runId, sourceId: "test", sourceKind: "upload",
      payloadType: "text/plain", contentPath: "/data/test.txt",
      contentHash: "abc", fetchedAt: new Date().toISOString(),
      metadataJson: "{}",
    });
    const extractionId = extractionRepo.save({
      documentId: docId, schemaVersion: "v1", promptVersion: "v1",
      modelName: "gpt-4o", status: "succeeded",
      rawJson: "{}", resultJson: '{"name": "Jane Doe", "bio": "Writer"}',
    });
    const subId = submissionRepo.insert({
      documentId: docId, extractionId, sourceId: "test",
    });
  ...
  it("parses SubmissionPacket format with candidates array", () => {
    ...
    const docId = `doc-${Math.random().toString(36).slice(2, 8)}`;
    rawRecordRepo.insertOne({
      id: docId, runId, sourceId: "test", sourceKind: "upload",
      payloadType: "text/plain", contentPath: "/data/test.txt",
      contentHash: "abc", fetchedAt: new Date().toISOString(),
      metadataJson: "{}",
    });
