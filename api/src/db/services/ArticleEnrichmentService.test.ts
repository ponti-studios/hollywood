...
describe("ArticleEnrichmentService", () => {
  ...
  beforeEach(() => {
    ...
    svc = new ArticleEnrichmentService({ articleRepo, entityRepo, creditRepo, companyRelationRepo, extractionRepo });

    articleRepo.upsertArticle({ articleId, sourceId: "variety", url: "https://variety.com/a", licenseClass: "web_copyright", runId });
    rawRecordRepo.insertOne({
      id: rawRecordId, runId, sourceId: "variety", sourceKind: "feed", payloadType: "feed_xml",
      contentPath: "/data/raw.xml", contentHash: "hash", fetchedAt: new Date().toISOString(), metadataJson: "{}",
    });
  });
  ...
  it("reuses an existing entity matched by canonical name instead of creating a duplicate", () => {
    const existingId = entityRepo.upsert({
      sourceId: "wga", entityType: "person", name: "Jane Doe",
      canonicalName: "jane doe", licenseClass: "research_non_commercial",
    });

    const mentions = makeMentions({ people: [{ name: "Jane Doe", roleHint: null, relatedTo: [] }] });
    const result = svc.materializeMentions(baseParams(mentions));
