...
describe("CompanyRelationRepository", () => {
  ...
  beforeEach(() => {
    ...
    companyId = entityRepo.upsert({
      sourceId: "test", entityType: "company", name: "Netflix",
      canonicalName: "netflix", companyType: "streamer", licenseClass: "public",
    });
    titleId = entityRepo.upsert({
      sourceId: "test", entityType: "title", name: "The Crown",
      canonicalName: "the crown", licenseClass: "public",
    });
  });
  ...
  it("is idempotent for the same (companyAId, entityId, relationship)", () => {
    const id1 = repo.upsert({ companyAId: companyId, entityType: "title", entityId: titleId, relationship: "producing", sourceId: "test" });
    const id2 = repo.upsert({ companyAId: companyId, entityType: "title", entityId: titleId, relationship: "producing", sourceId: "test" });
  ...
  it("allows a different relationship for the same pair as a separate row", () => {
    repo.upsert({ companyAId: companyId, entityType: "title", entityId: titleId, relationship: "producing", sourceId: "test" });
    repo.upsert({ companyAId: companyId, entityType: "title", entityId: titleId, relationship: "distributing", sourceId: "test" });

    expect(repo.findByCompany(companyId)).toHaveLength(2);
  });

  it("accepts a custom trustState", () => {
    repo.upsert({ companyAId: companyId, entityType: "title", entityId: titleId, relationship: "producing", sourceId: "test", trustState: "llm_extracted" });
